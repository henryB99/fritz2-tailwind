...
class Toggle(initializer: Initializer<Toggle>) : Component<Button> {
    ...
    override fun RenderContext.render(classes: String?, id: String?): Button {
        val componentId = id ?: value.id

        return button(
            "relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 ${classes.orEmpty()}",
            id = componentId
        ) {
